...
contract('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.king = await KingGhost.new(this.goex.address, dev, '1000', '0', '100000', { from: alice });
        this.migrator = await Migrator.new(this.king.address, this.factory1.address, this.factory2.address, '0');
        await this.goex.transferOwnership(this.king.address, { from: alice });
        await this.king.add('100', this.lp1.address, true, { from: alice });
    });
    ...
    it('should do the migration successfully', async () => {
        ...
        assert.equal((await this.lp1.balanceOf(this.king.address)).valueOf(), '2000000');
        ...
        assert.equal((await this.lp2.balanceOf(this.king.address)).valueOf(), '2000000');
