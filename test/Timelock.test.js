...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should not allow non-owner to do operation', async () => {
        await this.goex.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.goex.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.goex.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.goex.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.goex.owner()).valueOf(), carol);
    });
    ...
    it('should also work with KingGhost', async () => {
        ...
        await this.king.add('100', this.lp1.address, true);
        await this.king.transferOwnership(this.timelock.address, { from: alice });
        const eta = (await time.latest()).add(time.duration.days(4));
        await this.timelock.queueTransaction(
            this.king.address, '0', 'set(uint256,uint256,bool)',
