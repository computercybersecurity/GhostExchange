...
contract("GhostStaking", ([alice, bob, carol]) => {
  beforeEach(async () => {
    ...
    this.xGoex = await GhostStaking.new(this.ghost.address, { from: alice });
    this.ghost.mint(alice, "100", { from: alice });
    this.ghost.mint(bob, "100", { from: alice });
    this.ghost.mint(carol, "100", { from: alice });
  });
  ...
  it("should not allow enter if not enough approve", async () => {
    ...
    await this.ghost.approve(this.xGoex.address, "50", { from: alice });
    await expectRevert(
      this.xGoex.enter("100", { from: alice }),
      "ERC20: transfer amount exceeds allowance"
    );
    await this.ghost.approve(this.xGoex.address, "100", { from: alice });
  ...
  it("should work with more than one participant", async () => {
    await this.ghost.approve(this.xGoex.address, "100", { from: alice });
    await this.ghost.approve(this.xGoex.address, "100", { from: bob });
    ...
    assert.equal(
      (await this.ghost.balanceOf(this.xGoex.address)).valueOf(),
      "30"
    );
    // GhostStaking get 20 more GOEXs from an external source.
    await this.ghost.transfer(this.xGoex.address, "20", { from: carol });
