...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        await this.goex.delegate(dev, { from: dev });
        this.king = await KingGhost.new(this.goex.address, dev, '100', '0', '0', { from: alice });
        await this.goex.transferOwnership(this.king.address, { from: alice });
        this.lp = await MockERC20.new('LPToken', 'LP', '10000000000', { from: minter });
        this.lp2 = await MockERC20.new('LPToken2', 'LP2', '10000000000', { from: minter });
        await this.king.add('100', this.lp.address, true, { from: alice });
        ...
        await this.king.deposit(0, '100', { from: minter });
        assert.equal((await this.goex.totalSupply()).valueOf(), '110');
        assert.equal((await this.goex.balanceOf(minter)).valueOf(), '100');
        assert.equal((await this.goex.balanceOf(dev)).valueOf(), '10');
        // Transfer ownership to timelock contract
        this.timelock = await Timelock.new(alice, time.duration.days(2), { from: alice });
        this.gov = await GovernorAlpha.new(this.timelock.address, this.goex.address, alice, { from: alice });
        ...
        await expectRevert(
            this.king.add('100', this.lp2.address, true, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.gov.propose(
                [this.king.address], ['0'], ['add(uint256,address,bool)'],
