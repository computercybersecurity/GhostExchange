...
contract("KingGhost", ([alice, bob, carol, dev, minter, owner]) => {
  ...
  context("With ERC/LP token added to the field", () => {
    ...
    it("should give out GOMIXs only after farming time", async () => {
      ...
      assert.equal((await this.ghost.balanceOf(bob)).valueOf(), "1000");
    ...
    it("should not distribute GOMIXs if no one deposit", async () => {
      ...
      await this.king.depositFor(0, "10", bob, { from: alice }); // block 210
      assert.equal(
        (await this.ghost.totalSupply()).sub(initialSupply).valueOf(),
        "0"
      );
      ...
      assert.equal((await this.lp.balanceOf(bob)).valueOf(), "1010");
    });
    ...
    it("should distribute GOMIXs properly for each staker", async () => {
      ...
      assert.equal(
        (await this.ghost.balanceOf(this.king.address)).valueOf(),
        "4334"
      );
      assert.equal((await this.ghost.balanceOf(dev)).valueOf(), "1000");
      ...
      assert.equal((await this.ghost.balanceOf(dev)).valueOf(), "2000");
      ...
      assert.equal((await this.ghost.balanceOf(bob)).valueOf(), "11831");
      // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
      assert.equal((await this.ghost.balanceOf(carol)).valueOf(), "26568");
    ...
    it("should give proper GOMIXs allocation to each pool", async () => {
      ...
      assert.equal((await this.king.pendingGhost(0, alice)).valueOf(), "11666");
      await time.advanceBlockTo("430");
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      assert.equal((await this.king.pendingGhost(0, alice)).valueOf(), "13333");
    ...
    it("should stop giving bonus GOMIXs after the bonus period ends", async () => {
      ...
      assert.equal((await this.king.pendingGhost(0, alice)).valueOf(), "10500");
      // At block 606, Alice withdraws all pending rewards and should get 10600.
      await this.king.deposit(0, "0", { from: alice });
      assert.equal((await this.king.pendingGhost(0, alice)).valueOf(), "0");
      assert.equal((await this.ghost.balanceOf(alice)).valueOf(), "10600");
    });
  });
});
